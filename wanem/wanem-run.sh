...
# 2. Create WAN zone conditions
    ...
    echo $wan_delay $wan_jitter $wan_lost

    # Set new NetEm conditions on Inside interface (for inbound traffic)
    if [ $wan_jitter = 0 ]
    then
        sudo tc qdisc add dev $in_interface parent 1:2 handle 20: netem delay $((wan_delay/2))ms loss $wan_loss
    else
		sudo tc qdisc add dev $in_interface parent 1:2 handle 20: netem delay $((wan_delay/2))ms $wan_jitter 25% distribution normal loss $wan_loss
    fi
...
# 3. Create Internet zone conditions
    ...
    if [ $int_jitter = 0 ]
    then
        sudo tc qdisc add dev $in_interface parent 1:3 handle 30: netem delay $((int_delay/2))ms loss $int_loss
    else
		sudo tc qdisc add dev $in_interface parent 1:3 handle 30: netem delay $((int_delay/2))ms $int_jitter 25% distribution normal loss $int_loss
    fi
